...
async fn create_market(
    ...
    // Create collateral mint using lib function
    let collateral_mint =
        lib::create_mint(&rpc_client, &payer_keypair, payer_keypair.pubkey()).await?;
    println!("Collateral mint created: {}", collateral_mint);

    // Derive all PDAs using market_id
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command {
        Commands::CreateMarket {
            ...
            create_market(program, payer, market_id, metadata, end_time).await?;
        }
    }
